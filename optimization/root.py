...
class Root:
    ...
    def levy_distribution(self, step=0.001):
        a = 1.5 * gamma(1.5) * sin(pi * 1.5 / 2) * (1 / pi)
        levy = a * step ** (-1 * (1 + 1.5))
        return levy
    ...
    def obl(self, pop, g_best):
        new_obl_pop = []
        for pos in pop:
            obl_pop = self.create_opposition_position(pos[self.ID_POS], g_best)
            obl_fit = self.get_fitness_position(obl_pop)
            new_obl_pop.append([obl_pop, obl_fit])
        return new_obl_pop
